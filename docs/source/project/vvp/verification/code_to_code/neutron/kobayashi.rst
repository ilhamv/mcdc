...
Note that this verification case retains the non-fissioning benchmark setup; the :ref:`pulsed tutorial <example_kobayashi_td>` instead demonstrates an extension with a fuel sphere and an outlet detector, so the comparison results on this page do not verify those additions.
